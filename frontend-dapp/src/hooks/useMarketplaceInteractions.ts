...
import { translateBlockchainError } from '@/utils/errors';
...
export const useMarketplaceInteractions = () => {
  ...
  const approveMarketplace = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const userFriendlyMessage = translateBlockchainError(error);
      toast.error(`Errore durante l'approvazione: ${userFriendlyMessage}`, { id: loadingToastId });
      return false;
    } finally {
      setIsLoading(false);
    }
  ...
  const listForSale = useCallback(async (tokenId: bigint, priceEth: string) => {
    ...
    try {
      ...
      const userFriendlyMessage = translateBlockchainError(error);
      toast.error(`Errore durante la messa in vendita: ${userFriendlyMessage}`, { id: loadingToastId });
      throw error;
    } finally {
      setIsLoading(false);
    }
  ...
  const startAuction = useCallback(async (tokenId: bigint, minPriceEth: string, durationSeconds: number) => {
    ...
    try {
      ...
      const userFriendlyMessage = translateBlockchainError(error);
      toast.error(`Errore durante l'avvio dell'asta: ${userFriendlyMessage}`, { id: loadingToastId });
      throw error;
    } finally {
      setIsLoading(false);
    }
  ...
  const removeFromSale = useCallback(async (tokenId: bigint) => {
    ...
    try {
      ...
      const userFriendlyMessage = translateBlockchainError(error);
      toast.error(`Errore durante la revoca della vendita: ${userFriendlyMessage}`, { id: loadingToastId });
      throw error;
    } finally {
      setIsLoading(false);
    }
  ...
  const purchaseNFT = useCallback(async (tokenId: bigint, priceEth: string) => {
    ...
    try {
      ...
      const userFriendlyMessage = translateBlockchainError(err);
      toast.error(`Acquisto fallito: ${userFriendlyMessage}`, { id: toastId });
      throw err;
    } finally {
      setIsLoading(false);
    }
  ...
  const placeBid = useCallback(async (tokenId: bigint, bidAmountEth: string) => {
    ...
    try {
      ...
      const userFriendlyMessage = translateBlockchainError(err);
      toast.error(`Offerta fallita: ${userFriendlyMessage}`, { id: toastId });
      throw err;
    } finally {
      setIsLoading(false);
    }
  ...
  const claimAuction = useCallback(async (tokenId: bigint) => {
    ...
    try {
      ...
      const userFriendlyMessage = translateBlockchainError(err);
      toast.error(`Reclamo fallito: ${userFriendlyMessage}`, { id: toastId });
      throw err;
    } finally {
      setIsLoading(false);
    }
  ...
  const claimRefund = useCallback(async (tokenId: bigint) => {
    ...
    try {
      ...
      const userFriendlyMessage = translateBlockchainError(err);
      toast.error(`Richiesta rimborso fallita: ${userFriendlyMessage}`, { id: toastId });
      throw err;
    } finally {
      setIsLoading(false);
    }
