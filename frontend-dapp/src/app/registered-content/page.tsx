...
const RegisteredContentPage = () => {
  ...
  const buildContentTokenMapping = useCallback(async () => {
    ...
    try {
      ...
      // Fallback: usa il metodo tradizionale se gli eventi falliscono
      setIsMappingInitialized(true);
    }
  ...
  const fetchRegisteredContents = useCallback(async () => {
    ...
    try {
      // Costruisci/aggiorna la mappatura se necessario
      ...
      for (let i = 1; i <= totalContents; i++) {
        try {
          ...
          if (content.mintedCopies > BigInt(0)) {
            try {
              // OTTIMIZZAZIONE: Usa la mappatura invece di iterare
              const contentKey = i.toString();
              const tokenIds = contentTokenMapping.get(contentKey);

              if (tokenIds && tokenIds.length > 0) {
                // Prendi il primo tokenId disponibile
                ...
                try {
                  ...
                  console.warn(
                    `Errore nel recupero del token ${firstTokenId}:`,
                    tokenError
                  );
                }
              } else {
                // Fallback: usa il metodo tradizionale se la mappatura è vuota
                console.warn(
  ...
  useEffect(() => {
    if (isConnected && chainId === ARBITRUM_SEPOLIA_CHAIN_ID && publicClient) {
      ...
      const intervalId = setInterval(async () => {
        // Aggiorna la mappatura ogni minuto
        await buildContentTokenMapping();
        fetchRegisteredContents();
      }, 60000);
...
//           if (content.mintedCopies > BigInt(0)) {
//             try {
//               // SOLUZIONE: Trova il primo tokenId che corrisponde a questo contentId
//               const totalSupply = await nftContract.read.totalSupply();
//               let foundTokenId: bigint | null = null;

//               // Cerca tra tutti i token esistenti
...
//                   if (nftMetadata.contentId === BigInt(i)) {
//                     foundTokenId = BigInt(tokenId);
//                     break; // Prendi il primo token trovato per questo contentId
//                   }
//                 } catch (tokenError) {
//                   // Token potrebbe non esistere o essere inaccessibile, continua
//                   continue;
//                 }
//               }
