...
const SellAuctionModal = ({
  ...
  const validateDuration = (value: string) => {
    ...
    if (durationInSeconds < minDurationInSeconds) {
      setDurationError(
        `Durata minima ${MIN_AUCTION_DURATION_MINUTES_FRONTEND} minuti (${(
          MIN_AUCTION_DURATION_MINUTES_FRONTEND / 60
        ).toFixed(2)} ore)`
      );
      return false;
    }
  ...
  const handleStartSale = () => {
    ...
    if (isAuction && (!duration || !validateDuration(duration))) {
      toast.error("Inserisci una durata valida per l'asta.");
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md mx-4 border border-purple-600">
        ...
        <p className="text-gray-300 text-sm mb-4">
          Stai per mettere in vendita l'NFT "{nft.title}" (ID:{" "}
        ...
        <div className="mb-6">
          ...
          <Input
            ...
            className="bg-gray-700 text-white border border-gray-500 rounded-md p-3 text-sm focus:border-purple-400 focus:ring-purple-400"
          />
          {priceError && (
        ...
        {isAuction && (
          <div className="mb-6">
            ...
            <Input
              ...
              min="0.25"
              max="720"
              placeholder="es. 24 (min. 0.25)"
              value={duration}
              onChange={(e) => {
                setDuration(e.target.value);
                if (e.target.value) validateDuration(e.target.value);
              }}
              className="bg-gray-700 text-white border border-gray-500 rounded-md p-3 text-sm focus:border-purple-400 focus:ring-purple-400"
            />
            {durationError && (
        ...
        <div className="flex space-x-3 mt-6">
          ...
          <Button
            ...
            disabled={
              !price ||
              !!priceError ||
              (isAuction && (!!durationError || !duration))
            }
            className="flex-1 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-500 disabled:cursor-not-allowed"
          >
...
export default function MyNFTsPage() {
  ...
  const filteredNfts = useMemo(() => {
    const displayable = allMyNfts.filter(
  ...
  const handleRevokeSale = useCallback(
    async (nft: NFT) => {
      try {
  ...
  const handleTransfer = useCallback(
    async (nft: NFT) => {
      ...
      if (!walletClient || !address || !publicClient || !chainId) {
        toast.error("Wallet o client non disponibili per il trasferimento.");
        return;
      }
      ...
      try {
        ...
        pendingDetails = buildPendingTxDetails(
          ...
          chainId,
          { tokenId: nft.tokenId.toString(), recipient }
        );
        ...
        // Ora hash è accessibile qui
        const fallbackHash = (hash || "0x" + "0".repeat(64)) as `0x${string}`;
        const fallbackPendingDetails = pendingDetails || {
          ...
          metadata: { tokenId: nft.tokenId.toString(), recipient },
        };
        const failedDetails = buildFailedTxDetails(fallbackPendingDetails, err);
        await trackTransaction(failedDetails);
        toast.error(
          `Errore nel trasferimento: ${err.shortMessage || err.message}`,
          { id: toastId }
        );
      } finally {
  ...
  if (!isConnected)
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-center">
        <h1 className="text-2xl mb-4 text-purple-400">
          Connetti il tuo Wallet
        </h1>
        <p className="mb-4 text-gray-400">
          Per visualizzare i tuoi NFT, connetti il wallet.
        </p>
        <ConnectButton />
      </div>
    );
  if (!isChainCorrect)
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-center text-red-400">
        <h1 className="text-2xl mb-4">Rete Errata</h1>
        <p className="text-gray-400">Connetti a Arbitrum Sepolia.</p>
      </div>
    );
                ...
                <div className="overflow-x-auto">
                  <Table className="min-w-full divide-y divide-gray-700">
                    <TableHeader className="bg-gray-700/50">
                      <TableRow>
                        <TableHead className="px-3 py-3 text-left text-sm font-semibold text-gray-200">
                          Token ID
                        </TableHead>
                        <TableHead className="px-3 py-3 text-left text-sm font-semibold text-gray-200">
                          Titolo
                        ...
                        <TableHead className="px-3 py-3 text-center text-sm font-semibold text-gray-200">
                          Speciale
                        </TableHead>
                        <TableHead className="px-3 py-3 text-center text-sm font-semibold text-gray-200">
                          Azioni
                    ...
                    <TableBody className="bg-gray-900 divide-y divide-gray-700">
                      {currentNfts.map((nft) => {
                        ...
                        return (
                          <TableRow
                            key={nftIdString}
                            className="hover:bg-gray-700/80 transition-colors cursor-pointer"
                            onClick={() => {
                              window.location.href = `/nft-details/${nftIdString}`;
                            }}
                          >
                            <TableCell className="px-3 py-4 font-mono text-purple-300">
                              {nftIdString}
                            </TableCell>
                            ...
                            <TableCell className="px-3 py-4">
                              <Image
                                src={resolveIpfsLink(nft.imageUrlFromMetadata!)}
                                alt={nft.title!}
                                width={64}
                                height={64}
                            ...
                            <TableCell
                              className="px-3 py-4 w-64"
                              onClick={(e) => e.stopPropagation()}
                            >
                              <div className="flex flex-col space-y-2 items-center">
                                {canSell && (
                                  <Button
                                    onClick={() => handleSellAuctionClick(nft)}
                                ...
                                {isTokenInAuction && (
                                  <Button
                                    disabled
                                    className="bg-gray-500 w-full text-xs cursor-not-allowed"
                                  >
                                    In Asta
                                  </Button>
                                )}
                                {canTransfer && ( 
                                  <div className="flex items-center space-x-2 pt-2 w-full">
                                    <Input
                                      ...
                                      className="bg-gray-700 text-xs p-1 h-8 flex-grow"
                                    />
                                    <Button
                                      ...
                                      disabled={
                                        ...
                                        !isAddress(
                                          transferAddressInput.get(
                                            nftIdString
                                          ) || ""
                                        )
                                      }
                                      className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-500 text-xs px-2 h-8"
                                    >
                                      {" "}
                                      {isCurrentlyTransferring
                                        ? "..."
