...
// import { toast, Toaster } from "react-hot-toast";
// import { Address, isAddress } from "viem";
// import { ARBITRUM_SEPOLIA_CHAIN_ID, SCIENTIFIC_CONTENT_NFT_ABI, SCIENTIFIC_CONTENT_NFT_ADDRESS } from "@/lib/constants";
// import { ConnectButton } from "@rainbow-me/rainbowkit";
// import Image from "next/image";
// import {
//   Card,
...
// const ITEMS_PER_PAGE = 10; // Quanti NFT mostrare per pagina
...
// // Componente Toggle Switch per la modalità vendita/asta
// const ToggleSwitch = ({ isAuction, onToggle }: { isAuction: boolean; onToggle: () => void }) => (
//   <div className="flex items-center space-x-3">
//     <span className={`text-sm ${!isAuction ? 'text-purple-400 font-semibold' : 'text-gray-400'}`}>
//       VENDITA IMMEDIATA
//     </span>
//     <button
//       onClick={onToggle}
...
// // Componente Modale per Vendita/Asta
// const SellAuctionModal = ({
//   nft,
//   isOpen,
//   onClose
...
//       alert(`ASTA AVVIATA!\nNFT ID: ${nft?.tokenId.toString()}\nPrezzo di partenza: ${price} ETH\nTitolo: ${nft?.title}`);
//     } else {
//       alert(`VENDITA IMMEDIATA AVVIATA!\nNFT ID: ${nft?.tokenId.toString()}\nPrezzo: ${price} ETH\nTitolo: ${nft?.title}`);
//     }

//     // Reset del form e chiusura modale
//     setPrice("");
//     setPriceError("");
//     setIsAuction(false);
//     onClose();
...
//           </h2>
//           <p className="text-gray-300 text-sm mb-4">
//             Stai per mettere in vendita il tuo NFT "{nft.title}" (ID: {nft.tokenId.toString()}).
//             Scegli la modalità di vendita e imposta il prezzo.
//           </p>
//         </div>

//         {/* Toggle per modalità vendita/asta */}
//         <div className="mb-6">
//           <label className="block text-sm font-medium text-gray-300 mb-3">
//             Modalità di vendita
//           </label>
...
// export default function MyNFTsPage() {
//   // Rimosse le dichiarazioni di mounted e useEffect relativi
...
//   const [isValidAddress, setIsValidAddress] = useState<Map<string, boolean>>(new Map());
//   const [isTransferring, setIsTransferring] = useState<Map<string, boolean>>(new Map());
//   const [transferStatus, setTransferStatus] = useState<Map<string, string>>(new Map());

//   // Stato per la paginazione
//   const [currentPage, setCurrentPage] = useState(1);

//   // Stato per la modale di vendita/asta
//   const [isModalOpen, setIsModalOpen] = useState(false);
//   const [selectedNft, setSelectedNft] = useState<OwnedNFT | null>(null);

//   const isChainCorrect = chainId === ARBITRUM_SEPOLIA_CHAIN_ID;

//   // Filtra gli NFT visualizzabili e li memorizza con useMemo per evitare ricalcoli inutili
//   const displayableNfts = useMemo(() => {
//     return ownedNfts.filter((nft) => {
//       // Un NFT è visualizzabile se ha titolo, descrizione, e l'hash del contenuto IPFS
//       // e idealmente, anche un'immagine valida dai metadati.
//       const hasContentMetadata = nft.title && nft.description && nft.contentIpfsHash;
//       const hasValidImage = nft.imageUrlFromMetadata; // Verifichiamo solo se l'URL è presente
//       return hasContentMetadata && hasValidImage;
//     });
//   }, [ownedNfts]);

//   // Logica di paginazione
//   const totalPages = Math.ceil(displayableNfts.length / ITEMS_PER_PAGE);
//   const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
//   const endIndex = startIndex + ITEMS_PER_PAGE;
//   const currentNfts = displayableNfts.slice(startIndex, endIndex);
...
//   const handleTransferAddressChange = useCallback((tokenId: bigint, value: string) => {
//     setTransferAddressInput(prev => {
//       const newMap = new Map(prev);
//       newMap.set(tokenId.toString(), value);
//       return newMap;
//     });
//     setIsValidAddress(prev => {
//       const newMap = new Map(prev);
//       newMap.set(tokenId.toString(), isAddress(value));
//       return newMap;
//     });
//   }, []);
...
//   const handleTransfer = useCallback(async (nft: OwnedNFT) => {
//     const nftIdString = nft.tokenId.toString();
//     const recipient = transferAddressInput.get(nftIdString);

//     if (!recipient || !isAddress(recipient)) {
//       toast.error("Inserisci un indirizzo Ethereum valido per il trasferimento.");
//       return;
//     }
...
//       return newMap;
//     });
//     setTransferStatus(prev => new Map(prev).set(nftIdString, "Iniziando il trasferimento..."));
...
//       toast.loading(`Transazione inviata: ${hash.slice(0, 6)}...${hash.slice(-4)}. In attesa di conferma...`, {
//         id: loadingToastId,
//       });
//       setTransferStatus(prev => new Map(prev).set(nftIdString, `Transazione inviata, in attesa di conferma: ${hash.slice(0, 6)}...${hash.slice(-4)}`));

//       const transactionReceipt = await publicClient.waitForTransactionReceipt({ hash });

//       if (transactionReceipt.status === 'success') {
//         toast.success(`NFT ID ${nft.tokenId.toString()} trasferito con successo! Transazione: ${hash.slice(0, 6)}...${hash.slice(-4)}`, {
//           id: loadingToastId,
//         });
//         setTransferStatus(prev => new Map(prev).set(nftIdString, "Trasferimento completato con successo!"));
//         setTransferAddressInput(prev => {
//           const newMap = new Map(prev);
//           newMap.delete(nftIdString);
//           return newMap;
...
//         refetchOwnedNfts();
//       } else {
//         toast.error(`Trasferimento fallito per NFT ID ${nft.tokenId.toString()}.`, {
//           id: loadingToastId,
//         });
//         setTransferStatus(prev => new Map(prev).set(nftIdString, "Trasferimento fallito o annullato. Controlla la transazione."));
//       }

//     } catch (err: any) {
//       toast.error(`Errore nel trasferimento: ${err.shortMessage || err.message || "Operazione fallita"}`, {
//         id: loadingToastId,
//       });
//       setTransferStatus(prev => new Map(prev).set(nftIdString, `Errore: ${err.shortMessage || err.message || "Operazione fallita"}`));
//     } finally {
//       setIsTransferring(prev => {
//         const newMap = new Map(prev);
//         newMap.set(nftIdString, false);
...
//       });
//     }
//   }, [address, walletClient, publicClient, transferAddressInput, refetchOwnedNfts]);

//   // Rimosso il blocco if (!mounted) { return null; }
...
//   return (
//     <div className="bg-gray-200 text-white min-h-screen p-4">
//       <Toaster position="top-right" />
...
//           </CardTitle>
//           <CardDescription className="text-gray-400">
//             Qui trovi tutti gli NFT di contenuto scientifico che possiedi sul tuo
//             wallet collegato.
//           </CardDescription>
//         </CardHeader>
//         <CardContent>
...
//                 </TableHeader>
//                 <TableBody className="bg-gray-900 divide-y divide-gray-700">
//                   {currentNfts.map((nft) => { // Usa currentNfts per la paginazione
//                     const isTokenTransferring = isTransferring.get(nft.tokenId.toString()) || false;
//                     const isTransferButtonDisabled = !isValidAddress.get(nft.tokenId.toString()) || isTokenTransferring;
//                     const currentTransferStatus = transferStatus.get(nft.tokenId.toString());

//                     // Usa imageUrlFromMetadata, risolvendolo con resolveIpfsLink
//                     const imageUrl = nft.imageUrlFromMetadata ? resolveIpfsLink(nft.imageUrlFromMetadata) : PLACEHOLDER_IMAGE_URL;
...
//                           <Image
//                             src={imageUrl}
//                             alt={`Immagine di Copertina di ${nft.title || 'NFT'}`}
//                             width={80}
//                             height={80}
//                             className="rounded-md object-cover"
//                             onError={(e) => { // Gestione dell'errore per immagini non caricate
//                                 e.currentTarget.src = PLACEHOLDER_IMAGE_URL;
...
//                                 type="text"
//                                 placeholder="Indirizzo Destinatario"
//                                 value={transferAddressInput.get(nft.tokenId.toString()) || ""}
//                                 onChange={(e) => handleTransferAddressChange(nft.tokenId, e.target.value)}
//                                 className="bg-gray-700 text-white border border-gray-500 rounded-md p-1 text-sm focus:border-purple-400 focus:ring-purple-400"
//                               />
//                               <Button
//                                 onClick={() => handleTransfer(nft)}
