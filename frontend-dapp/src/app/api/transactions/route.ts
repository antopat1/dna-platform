...
const TARGET_DB_NAME = "DnaContentMarketplaceDB"; 
const TARGET_COLLECTION_NAME = "events"; 
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (status === "event_minted") {
            ...
            const eventName = args?.event || "UnknownEvent"; 
            ...
            recordToSave = {
                ...
                timestamp_processed: new Date(timestamp || Date.now()), 
            ...
            if (recordToSave.logIndex === null) {
                ...
                filterQuery = {
                    transactionHash: transactionHash,
                    'metadata.status': "event_minted_fallback" 
                };
            ...
            const transactionRecordId = `${transactionHash}_frontend_tx_status`; 
...
//$$$$ DISABILITA PROVVISORIAMANETE IL SALVATAGGIO SU MONGODB DAL FRONTEND ( test script Container su Fly.io eventi OnChain fuori dalla piattaforma ) 
...
// export async function POST(req: NextRequest) {
//     try {
//         // --- INIZIO BLOCCO: CODICE TEMPORANEAMENTE COMMENTATO PER DISABILITARE IL SALVATAGGIO SU MONGODB DAL FRONTEND ---
//         /*
//         const {
//             transactionHash,
//             from,
...
//         console.log(`Record for transaction ${transactionHash} (status: ${status}) saved/updated.`, result.upsertedId || result.modifiedCount);
//         */
//         // --- FINE BLOCCO: CODICE TEMPORANEAMENTE COMMENTATO ---


//         // --- INIZIO BLOCCO: CODICE PER IL COMPORTAMENTO TEMPORANEO DI SKIP ---
//         // Usiamo valori placeholder per le variabili che sarebbero state destrutturate
//         // e rinominiamo 'status' in 'processingStatus' per evitare il warning di deprecazione nel payload.
//         const mockTransactionHash = '0x0000000000000000000000000000000000000000'; // Un hash fittizio
//         const mockProcessingStatus = 'skipped_by_frontend_api'; // Uno stato fittizio e descrittivo

//         console.log(`Frontend API: Temporarily skipping saving to MongoDB for transaction. Mocking response.`);
//         return NextResponse.json(
//             {
//                 message: 'Transaction data processing (currently skipped) successfully',
//                 transactionHash: mockTransactionHash, // Usa il placeholder
//                 processingStatus: mockProcessingStatus // Usa il nome rinominato per lo stato
//             },
//             { status: 200 } // Questo 'status' è per la risposta HTTP ed è corretto
//         );
//         // --- FINE BLOCCO: CODICE PER IL COMPORTAMENTO TEMPORANEO DI SKIP ---

//     } catch (error) {
//         console.error('Error in frontend transaction API (even if skipped):', error);
//         return NextResponse.json({ error: 'Failed to process transaction data (due to internal error, not skipping)', details: (error as Error).message }, { status: 500 });
//     }
// }
